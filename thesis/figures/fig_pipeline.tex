...
\node[pstep] (raw) {zarr \texttt{wind}\\$[H,W,T]$\\\scriptsize(winding \emph{rate})};
...
\end{tikzpicture}
\caption[Data pipeline]{\textbf{Winding-flux data pipeline.} The stored field is a
near-white \emph{rate}; missing frames (sentinel timestamps) are removed, and the
field is accumulated over time into the persistent \emph{total} winding
$W[t]$ (Eq.~\ref{eq:cumsum}), the forecasting target. The total is passed through
