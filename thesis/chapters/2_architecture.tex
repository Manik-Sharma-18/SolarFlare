...
integrating the per-frame winding over time,
\begin{equation}
...
choice, and the data motivate it directly. The winding field carries appreciable
power at every spatial scale, right down to the pixel: its radially-averaged
power spectrum falls off only slowly and never flattens into a noise floor
(Figure~\ref{fig:psd}). That fine-scale content is not, however, useful for
