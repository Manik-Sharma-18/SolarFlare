...
Finally we reframe the objective toward autoregressive \emph{curve}
prediction (the spatial-mean winding-flux trajectory) and run a second
arm family (S46--S52: low-pass loss, 4\(\times\) spatial pooling,
scheduled sampling, a trajectory loss, and longer-rollout training).
A robust $\sim$30-window bootstrap of the spatial-mean curve correlation
on HARP 11930 shows \textbf{every arm's 95\% confidence interval straddles
zero} --- including the arm trained directly at the evaluation horizon.
We conclude this is a \emph{genuine predictability ceiling}: the
ground-truth spatial-mean curve is mean-reverting with near-white
deviations, so the optimal forecast is its climatological mean.
Pivoting from forecasting to \emph{classification}, we show the same
data supports a viable flare-forecasting task: a leave-AR-out classifier
predicting \(\ge\)M-class flares within 12--24\,h reaches ROC-AUC
\(\approx\)0.72 from \textbf{winding/helicity flux features alone}
(\(\approx\)95\% of the full physics set; helicity is the key carrier).
Winding flux is thus a flare-\emph{classification} signal, not a
forecasting target.
\end{abstract}
...
\input{setup.tex}
\newpage
\input{regression_arms.tex}
\newpage
\input{dualhead_arms.tex}
\newpage
\input{inference_combo.tex}
\newpage
\input{staircase.tex}
\newpage
\input{structural_arms.tex}
\newpage
\input{new_data_arms.tex}
\newpage
\input{curve_objective.tex}
\newpage
\input{flare_classification.tex}
\newpage
